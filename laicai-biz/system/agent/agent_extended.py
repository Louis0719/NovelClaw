...
来财AI客服 - 全功能版
服务叠石桥面料公司全公司6个部门
在 agent.py 基础上扩展所有部门功能
用法: python3 agent_extended.py
"""
import re, sqlite3
...
def h_sales_report(t):
    """销售报表"""
    r = qx.get_sales_report()
    return qx.format_sales_report(r)

def h_staff_performance(t):
    """员工绩效"""
    staff = qx.get_staff_performance()
    return qx.format_staff_performance(staff)

def h_qc_report(t):
    """质检报告"""
    checks = qx.get_quality_checks(limit=20)
    return f"🔍 质检报告\n\n{qx.format_qc_report(checks)}"
...
def h_production_delay(t):
    ...
    lines = ["⚠️ 即将延期/已延期订单"]
    for r in rows:
        lines.append(f"  🔴 {r['id']} | {r['customer']} | 交期:{r['delivery']}")
    return '\n'.join(lines)
...
def h_replenish(t):
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT p.id, p.name, p.stock, 1000 as alert_level
        FROM products p
        WHERE p.stock < 1000
        ORDER BY (1000 - p.stock) DESC
        LIMIT 10
    """)
    rows = cur.fetchall()
    conn.close()
    ...
    lines = ["📦 补货建议"]
    for r in rows:
        deficit = r['alert_level'] - r['stock']
        lines.append(f"  🔴 {r['name']}({r['id']}) 当前:{r['stock']}米 | 警戒:{r['alert_level']}米 | 建议补:{deficit}米")
    return '\n'.join(lines)
...
def h_complaints(t):
    ...
    lines = ["🔴 质量投诉记录"]
    for r in rows:
        lines.append(f"  ❌ {r['check_date']} | {r['product_id']} | {r['defect_type']} | {r['notes']}")
    return '\n'.join(lines)
...
# ========== 扩展意图识别 ==========
def get_extended_intent(t):
    t = t.strip()
    if any(k in t for k in ["日报", "今日情况", "今日经营", "今天情况", "经营情况", "驾驶舱"]):
        return "boss_dashboard"
    if any(k in t for k in ["报表", "销售报告", "月度", "季度", "销售统计"]):
        return "sales_report"
    if any(k in t for k in ["员工绩效", "谁做得好", "业务员排名", "业绩排行"]):
        return "staff_performance"
    if any(k in t for k in ["质检", "质量报告", "合格", "次品", "验货"]):
        return "qc_report"
    if any(k in t for k in ["物流", "单号", "运到", "快递"]):
        return "logistics"
    # 带订单号的生产进度优先匹配
    if re.search(r'DD-\d+-\d+', t) and any(k in t for k in ["进度", "进行", "到哪", "排期", "哪步"]):
        return "production"
    if any(k in t for k in ["生产进度", "排期"]):
        return "production"
    if any(k in t for k in ["延期", "逾期", "超时"]):
        return "production_delay"
    if any(k in t for k in ["库存预警", "库存提醒", "库存不足"]):
        return "inventory_alert"
    if any(k in t for k in ["发货安排", "今天发什么", "哪些要发货", "待发货"]):
        return "pending_shipment"
    if any(k in t for k in ["补货", "要进货", "建议补货"]):
        return "replenish"
    if any(k in t for k in ["客诉", "投诉", "质量投诉"]):
        return "complaints"
    if any(k in t for k in ["客户跟进", "跟进记录", "联系客户", "回访"]):
        return "customer_followup"
    if any(k in t for k in ["客户档案", "客户资料", "老客户"]):
...
def process(t):
    ...
    # 扩展意图优先
    ex_intent = get_extended_intent(t)
    if ex_intent:
        dispatch = {
            "boss_dashboard": h_boss_dashboard,
            "sales_report": h_sales_report,
            "staff_performance": h_staff_performance,
            ...
            "replenish": h_replenish,
            "complaints": h_complaints,
            "customer_followup": h_customer_followup,
            "customer_profile": h_customer_profile,
        }
        return dispatch[ex_intent](t)

    # VIP路由
    if is_vip_query(t) or any(k in t for k in ["印花", "染色", "涂层", "整理", "工艺"]):
        vip = check_vip(t)
        if vip:
    ...
    # 普通路由
    if any(p in t.lower() for p in ["面料","花型","色号","工艺","质量","地址","米数","备注"]):
        return h_new_order(t)
    if any(p in t.lower() for p in ["库存预警","库存告警","缺货"]):
        return h_alert()
    if any(p in t.lower() for p in ["有货","有没有","现货","多少米","还有多少","能发"]):
        return h_stock(t)
    if any(p in t.lower() for p in ["价格","多少钱","单价","报价"]):
        return h_price(t)
    # 特殊：订单+发货 -> 待发货清单（而非普通订单查询）
    if "订单" in t and "发货" in t:
        recs = qx.get_pending_shipments()
        return qx.format_pending_shipments(recs)
    if any(p in t.lower() for p in ["订单","货做好","进度","物流","交期"]):
        return h_order(t)
    if any(p in t.lower() for p in ["推荐","适合","爆款"]):
        return h_recommend(t)
    if any(p in t.lower() for p in ["帮助","怎么用","功能"]):
        return h_help()
    if any(p in t.lower() for p in ["订","下单","下一单","帮我订","要一批"]):
        return h_new_order(t)
